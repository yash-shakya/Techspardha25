...
import { Event } from '../../server/actions/constants'
type props = {
    events: (Event & { id: string })[],
    categories: string[]
}
export default function EventsClient({ events, categories }: props) {
    ...
    const filteredEvents = events.filter(event => {
        const matchesCategory = selectedCategory === "All Events" || event.eventCategory === selectedCategory
    ...
    return (
        <div className="bg-transparent px-4 py-8 md:px-8 lg:px-12">
            <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <h1 className="text-3xl font-bold  md:text-4xl pb-5  bg-gradient-to-b from-[#FDFDFD] to-[rgba(250, 250, 250, .1)] text-transparent bg-clip-text  font-[Satoshi Variable]">{selectedCategory}</h1>
            ...
            <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3 relative z-0">
                {filteredEvents.map((event: Event & {id: string}) => (
                    <div
                        key={event.id}
